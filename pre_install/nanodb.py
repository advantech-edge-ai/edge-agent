...
class NanoDB:
    ...
    def scan(self, path, max_items=None, **kwargs):
        ...
        for file in tqdm.tqdm(files, file=sys.stdout):
            with tqdm_redirect_stdout():
                ...
                filetag = None
                if "_filetag_" in file:
                    filetag = file.split("_filetag_")[-1].split(".")[0]
                self.metadata.insert(index, dict(path=file, tag=filetag))
    ...
    def delete(self, path):
        """
        Delete an entry from the database by its image path.
        
        Parameters:
            path (str): The path of the image to delete
            
        Returns:
            bool: True if deletion was successful, False if path not found
        ...
        # Find the index of the path in metadata
        target_index = None
        ...
        current_size = self.index.shape[0]
        
        if target_index < current_size - 1:
            # Move the last vector to the position of the deleted vector
            self.index.vectors.array[target_index] = self.index.vectors.array[current_size - 1]
            
            # If using L2 distance, also move the vector norms
            if self.index.metric == 'l2':
                self.index.vector_norms.array[target_index] = self.index.vector_norms.array[current_size - 1]
            
            # Update metadata
            self.metadata[target_index] = self.metadata[-1]
        
        # Remove the last element from metadata
        self.metadata.pop()
        
        # Update the shape to reflect the deletion
        self.index.shape = (current_size - 1, self.index.shape[1])
